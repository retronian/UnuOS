...
RELEASE_BETA=
RELEASE_BASE=UnuOS-$(RELEASE_TIME)$(RELEASE_BETA)
...
system:
	...
	cp ./workspace/all/unuos/build/$(PLATFORM)/unuos.elf ./build/SYSTEM/$(PLATFORM)/bin/
...
cores: # UnuOS: 7 bundled cores (no Extras concept)
...
package: tidy
	# ----------------------------------------------------
	# zip up build (single UnuOS package — no extras)
	...
	cd ./build/PAYLOAD && zip -r UnuOS.zip .system .tmp_update
	mv ./build/PAYLOAD/UnuOS.zip ./build/BASE

	# single zip (Tools now lives under BASE)
	cd ./build/BASE && zip -r ../../releases/$(RELEASE_NAME).zip Bios Roms Saves Tools miyoo miyoo354 trimui rg35xx rg35xxplus gkdpixel miyoo355 magicx miyoo285 em_ui.sh UnuOS.zip README.txt
