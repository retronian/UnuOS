...
if [ -d ${TF1_PATH}${SYSTEM_FRAG} ] || [ -f ${TF1_PATH}${UPDATE_FRAG} ]; then
	echo "found UnuUI on TF1" >> $TF1_PATH/log.txt
	if [ ! -h $TF2_PATH ]; then
...
if [ -f $UPDATE_PATH ]; then
	...
	*"480x640"*)
	...
	if [ ! -d $SYSTEM_PATH ]; then
		ACTION=installing
		echo "install UnuUI" >> $TF1_PATH/log.txt
	else
		ACTION=updating
		echo "update UnuUI" >> $TF1_PATH/log.txt
	fi
...
if [ -f $SYSTEM_PATH/paks/UnuUI.pak/launch.sh ]; then
	echo "launch UnuUI" >> $TF1_PATH/log.txt
	$SYSTEM_PATH/paks/UnuUI.pak/launch.sh
else
