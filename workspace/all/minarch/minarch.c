...
typedef struct Option {
	...
	// the core's category array. UnuOS stores the category display name
...
static bool environment_callback(unsigned cmd, void *data) { // copied from picoarch initially
	...
	case RETRO_ENVIRONMENT_GET_LANGUAGE: { /* 39 */
		// report UnuOS lang to the core so it can return localized option strings
		unsigned* out = (unsigned*)data;
...
void Menu_init(void) {
	...
	sprintf(menu.unuos_dir, SHARED_USERDATA_PATH "/.unuos/%s", emu_name);
	mkdir(menu.unuos_dir, 0755);

	sprintf(menu.slot_path, "%s/%s.txt", menu.unuos_dir, game.name);
...
MenuList options_menu = {
	.type = MENU_LIST,
	.items = (MenuItem[]) {
		{"Frontend", "UnuOS (" BUILD_DATE " " BUILD_HASH ")",.on_confirm=OptionFrontend_openMenu},
...
static void Menu_updateState(void) {
	...
	sprintf(menu.bmp_path, "%s/%s.%d.bmp", menu.unuos_dir, game.name, menu.slot);
	sprintf(menu.txt_path, "%s/%s.%d.txt", menu.unuos_dir, game.name, menu.slot);
