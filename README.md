...
- **Favorites and [Game Focus Mode](https://github.com/retronian/UnuOS/wiki/Game-Focus-Mode).** Press `Y` on a game to add or remove it from Favorites in the normal launcher. Press `SELECT` + `Y` to show only favorite games, then press `SELECT` + `Y` again to return to the normal launcher.
