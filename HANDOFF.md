# UnuOS Project Handoff

このドキュメントは UnuOS プロジェクトの現状を別の AI エージェントに引き継ぐためのものです。

## プロジェクトの目的

**UnuOS** は Miyoo Mini Plus 用のカスタムファームウェア。[MinUI](https://github.com/shauninman/MinUI) のフォークに以下を追加:

1. **日本語/CJK 対応**（フォント + UTF-8 テキスト処理）
2. **UI 多言語化**（7言語: en / ja / zh_cn / zh_tw / ko / es / fr）
3. **libretro コアオプションの多言語化**（コア側の翻訳テーブルを活用）
4. **UnuOS ブランディング**（Retronian ブランドの一部）
...
UnuOS = MinUI fork。RetroArch は使わず、独自の軽量 libretro frontend `minarch` を使う。
...
workspace/linux/         # ★UnuOS 独自: Linux 開発シミュレータ
...
make PLATFORMS=miyoomini        # フルパッケージ (releases/UnuOS-YYYYMMDD-N-base.zip 生成)
...
1. **Docker ownership**: ホストから mount されたソースは Docker 内で「dubious ownership」となり `git rev-parse` が失敗する。影響:
   ...
   - 対策: `workspace/miyoomini/cores/makefile` に `picodrive_FLAGS = GIT_REVISION=-unuos` を追加（実装済み）。
...
- **`GFX_truncateText()` の UTF-8 バグ修正** (`workspace/all/common/api.c:228`付近)
  ...
  - **NextUI 等他のフォークでは未修正**、UnuOS 独自の差別化ポイント
- 実機で日本語 ROM 名表示を確認済み

### ✅ Phase 2: UnuOS ブランディング
- `MinUI` → `UnuOS` リネーム（makefile RELEASE_BASE / minarch Frontend label / minui ログ / README）
- `skeleton/SYSTEM/res/charging-640-480.png` を One/OS 正方形ロゴに置換（ImageMagick）
- `workspace/miyoomini/install/installing.png` / `updating.png` も UnuOS 化
- tag `minui-fork-point` を dbf8943 に打った
- `CHANGELOG.md` 開始
- **注意**: 他プラットフォーム (rg35xx, trimui, etc.) のコードは残す方針。UnuOS は Miyoo Mini Plus 専用だが将来のマルチプラットフォーム展開余地は保つ。
...
- **Settings UI**: ランチャーのルートに仮想エントリ `SETTINGS_PATH` (`SDCARD/.unuos-settings`) を追加 →
...
- 結果: **UnuOS パッケージは `releases/UnuOS-YYYYMMDD-N-base.zip` 1 個のみ** (extras.zip は生成されない)
...
| gpsp | GBA | **ja/zh_cn/zh_tw/ko (UnuOS patch) — 13 options** |
| picodrive | Mega Drive / Genesis / 32X / Mega CD / SMS / Game Gear | **ja/zh_cn/zh_tw/ko (UnuOS patch) — 21 options** |
| snes9x2005_plus | SNES / Super Famicom | **ja/zh_cn/zh_tw/ko (UnuOS patch) — 7 options** |
| pcsx_rearmed | PlayStation | **ja/zh_cn/zh_tw/ko (UnuOS patch) — 全 73 options** |
...
**Docker を使わない開発を可能にするための UnuOS 独自の仕組み**:
...
26d6da5 phase 2: rebrand to UnuOS + linux dev simulator
...
7. **upstream PR** — UnuOS コア翻訳パッチ (snes9x2005_plus / gpsp / picodrive / pcsx_rearmed の ja/zh/ko) は upstream にも貢献価値あり。各 core リポジトリに PR 提出を検討
